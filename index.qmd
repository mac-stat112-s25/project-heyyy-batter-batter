...
=======
```{r}
#Home_runs vs park_effects

ggplot(joined_stadium_data, aes(x = hr_park_effects, y = home_run, size = hr_park_effects)) +
  geom_point(alpha = 0.7, color = "#2E86AB") +
  geom_smooth(method = "lm", se = FALSE, linetype = "dashed", color = "black") +
  scale_size_continuous(
    range = c(2, 6),        # small→large dot sizes
    name  = "HR Park Effect" # legend title
  ) +
  labs(
    title   = "Player Home Runs vs. Park Effects",
    x       = "Home Run Park Effect",
    y       = "Total Home Runs",
    caption = "Source: Baseball Savant 2022 & ballparks_stadium_data"
  ) +
  theme_minimal(base_family = "baseball_font") +
  theme(
    plot.title   = element_text(hjust = 0.5, face = "bold", size = 12),
    axis.title   = element_text(face = "bold", size = 10),
    axis.text    = element_text(size = 9),
